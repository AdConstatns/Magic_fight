...
namespace Apex.Examples.AI
{
    ...
    using UnityEngine;
#if UNITY_2020 || UNITY_2017 || UNITY_5_5 || UNITY_5_6
    ...
    public sealed class NextToBlockScorer : OptionScorerWithScore<Vector3>
    {
        ...
        public override float Score(IAIContext context, Vector3 pos)
        {
            ...
            NavMeshHit hit;
#if UNITY_5 || UNITY_2017 || UNITY_2020
            ...
            if (!NavMesh.SamplePosition(p1, out hit, Mathf.Epsilon, layer))
            {
                return this.score;
            }

            var p2 = new Vector3(pos.x + this.samplingDistance, 0f, pos.z);
            if (!NavMesh.SamplePosition(p2, out hit, Mathf.Epsilon, layer))
            {
                return this.score;
            }

            var p3 = new Vector3(pos.x, 0f, pos.z - this.samplingDistance);
            if (!NavMesh.SamplePosition(p3, out hit, Mathf.Epsilon, layer))
            {
                return this.score;
            }

            var p4 = new Vector3(pos.x, 0f, pos.z + this.samplingDistance);
            if (!NavMesh.SamplePosition(p4, out hit, Mathf.Epsilon, layer))
            {
                return this.score;
            }
