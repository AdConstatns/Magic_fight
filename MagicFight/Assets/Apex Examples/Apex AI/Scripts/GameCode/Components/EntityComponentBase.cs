...
namespace Apex.Examples.AI.Game
{
    ...
    public abstract class EntityComponentBase : MonoBehaviour, IAIEntity, IContextProvider
    {
        ...
        protected void Start()
        {
            ...
            var particlesContainer = GameObject.Find("ParticlesContainer");
            if (particlesContainer != null)
            {
                _bulletParticle.transform.SetParent(particlesContainer.transform);
            }
...
#else
            int mask = _navMeshAgent.walkableMask;
#endif
            if (NavMesh.SamplePosition(destination, out hit, 1f, mask))
            {
